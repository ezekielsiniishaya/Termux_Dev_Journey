	...
	while (array[row][column] != '\0') {
		if (array[row][column] != ' ' && array[row][column] != '\t') {
			if (array[row][column] == '/') {
				...
				printf("found comment %c", array[row][column]);
				return column;
				...
				printf("found %c", array[row][column]);
				return 1;
			}
		}
		column++;
...
void clear_array(char array[][MAX_COLUMNS], int row)
{
	...
	while (i < row) {
		...
		// Line starts with a comment
			...
			printf("character is %d", character);
			int check = find_character(array, i, character);
			printf("found check at %d", check);
			// Check for single line or multiple line comment
			if (check == 1) {
				...
				printf("found multiple at %d", multiple);
				if (multiple != -1) {
					int k = multiple + 2;
					for (int j = i; j < row; j++) {
