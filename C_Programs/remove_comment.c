/* This program removes all comments from a given source code input. */

#include <stdio.h>

// Constants for the maximum number of rows (lines) and columns (characters)
...
It returns:
   the column if the line starts with a /,
   -1 if it starts #,
   -2 if it starts *,
   -3 if it starts with any other non-whitespace character
   -4 if the line is empty or contains only whitespace.
*/
...
// This function removes the first line of multi-line comments ('/* ... ') from
// a given row of the code array.
...
/* This function processes the 2D array of code and removes both
   ...
   identifies comments, and removes them by modifying the array. It considers
   comments in strings and character constants*/
void clear_array(char array[][MAX_COLUMNS], int row) {
  ...
  while (i < row) {
    ...
    // Line starts with a comment
      ...
      int check = find_character(array, i, character + 1);
      // printf("on comment line. check is %d, and character is %d", check,
      // character); Check for single line or multiple line comment
      if (check != -2) {
...
It returns the index of the first non-whitespace character or -1 if the row is
empty. */
...
int main() {
  ...
  printf("Enter up to %d lines of code (each up to %d characters). Press "
         "'Ctrl+D' to stop:\n",
         MAX_ROWS, MAX_COLUMNS - 1);
  // Iterating through the code lines and saving into the array, checking that
  // they do not exceed the array size
